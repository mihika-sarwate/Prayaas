...
async function upsertEmployeeRowsToSupabase(rows) {
  ...
  try {
    ...
    if (String((err && err.message) || '').toLowerCase().indexOf('designation') !== -1) {
      console.warn('Supabase employees table is missing designation, retrying without that column.');
      await upsertInBatches('employees', stripDesignationFromRows(rows));
      return;
    }
    throw err;
  }
...
async function insertEmployeeRowsToSupabase(rows) {
  ...
  try {
    ...
    if (String((err && err.message) || '').toLowerCase().indexOf('designation') !== -1) {
      console.warn('Supabase employees table is missing designation, retrying without that column.');
      const res2 = await supabase.from('employees').insert(stripDesignationFromRows(rows));
...
async function upsertInBatches(table, dataArray, chunkSize = 1000) {
  ...
  for (let i = 0; i < deduped.length; i += chunkSize) {
    ...
    if (data) {
      allUpdatedRows = allUpdatedRows.concat(data);
    }
  }
...
async function initSupabaseData(isSilent) {
  ...
  if (!supabase) {
    ...
    ensureSupabaseLoaded(async function() {
      isLoadingData = false; // release so next call can proceed
      if (supabase) {
  ...
  try {
    ...
    DB = {
      employees: emps.map(e => {
        ...
        var existing = (typeof DB !== 'undefined' && DB && DB.employees) ? DB.employees.find(em => em.id === e.id) : null;
      ...
      doctors: (function() {
        ...
        var missingLocal = localDocs.filter(function(d) {
          return !cloudIds.has(String(d.id));
        });
      ...
      inputsInventory: (inputsInv || []).map(i => ({
        id: i.id,
    ...
    if (!hasInitializedData) {
      ...
      try {
        ...
        console.error("loadLocalDB failed in catch:", e);
      }
    }
    isLoadingData = false;
  }
...
window.addEventListener('beforeunload', function (e) {
  if (isSyncing) {
    e.preventDefault();
    e.returnValue = 'Data is currently syncing to the cloud. If you leave now, recent changes may be lost.';
  }
});
  ...
  async function syncSupabaseDatabase() {
    ...
    try {
      const dbDocs = DB.doctors.map(d => {
        ...
        if (assignTo) {
          var normAssign = String(assignTo).toUpperCase().trim();
          if (activeEmpIds.size > 0 && !activeEmpIds.has(normAssign)) {
            assignTo = null;
          } else if (activeEmpIds.size === 0 && !DB.employees.find(x => String(x.id || '').toUpperCase().trim() === normAssign)) {
            assignTo = null;
          }
        }
        return {
    ...
    try {
      ...
      }, { select: 'id,emp_id,from_loc,to_loc,distance,mode,category,fare,da,working_days,emp_name,hq,state,doctors,business' });
    } catch (e) {
...
function initEmpApp(){
  ...
  var defMonth=now.toISOString().slice(0,7);
...
function populateReportingDropdowns(){
  ...
  promoProductsGrid.innerHTML=DB.products.map(function(p, i){
...
function renderPlannedTargetsForReport(plannedDay){
  ...
  if(!wrap)return;
  var plannedDocIds = plannedDay ? ((plannedDay.plannedDocs&&plannedDay.plannedDocs.length) ? plannedDay.plannedDocs : getAssignedDoctorsForArea(plannedDay.areaTerritory||plannedDay.toLocation).map(function(doc){return doc.id;})) : [];
  var plannedChemIds = plannedDay ? ((plannedDay.plannedChems&&plannedDay.plannedChems.length) ? plannedDay.plannedChems : getAssignedChemistsForArea(plannedDay.areaTerritory||plannedDay.toLocation).map(function(chem){return chem.id;})) : [];
  if(!plannedDay || (!plannedDocIds.length && !plannedChemIds.length)){
    wrap.style.display='none';
    wrap.innerHTML='';
    return;
  }
  var docBtns=plannedDocIds.map(function(docId){
    var doc=DB.doctors.find(function(d){return d.id===docId && d.assignTo===SESSION.user.id && d.status==='Active';});
    return doc ? '<button type="button" class="btn sm" style="width:auto;margin:3px;display:inline-flex" onclick="selectPlannedReportTarget(\'Doctor\', \''+doc.id+'\')">'+doc.name+'</button>' : '';
  }).join('');
  var chemBtns=plannedChemIds.map(function(chemId){
    var chem=DB.chemists.find(function(c){return c.id===chemId && c.assignTo===SESSION.user.id;});
    return chem ? '<button type="button" class="btn sm" style="width:auto;margin:3px;display:inline-flex" onclick="selectPlannedReportTarget(\'Chemist\', \''+chem.id+'\')">'+chem.name+'</button>' : '';
  }).join('');
...
function isPendingForUser(status, managerId, u) {
  ...
  if (isMgr) {
    if (statusUpper === 'SUBMITTED' || statusUpper === 'APPROVED BY ADMIN') return true;
  }
  if (isAdmin) {
    if (statusUpper === 'SUBMITTED' || statusUpper === 'APPROVED BY MANAGER') return true;
  }
  return false;
}
...
function processApproval(currentStatus, managerId, u) {
  ...
  if (statusUpper === 'SUBMITTED') {
    if (isMgr && isAdmin) {
      ...
      return mgrId ? 'Approved by Admin' : 'Approved';
    }
  } else if (statusUpper === 'APPROVED BY MANAGER') {
    if (isAdmin) return 'Approved';
  } else if (statusUpper === 'APPROVED BY ADMIN') {
    if (isMgr) return 'Approved';
  }
...
async function reconcileSupabaseRows(table, localRows, localKeyFn, deleteRowFn, options) {
  ...
  try {
    ...
    const localKeys = new Set((localRows || []).map(localKeyFn));
    const staleRows = remoteRows.filter(function(row) {
      return !localKeys.has(localKeyFn(row));
    });
...
function updateReportClassificationFromTarget(){
  ...
  var targetId=targetType==='Doctor' ? document.getElementById('r-doctor').value : document.getElementById('r-chemist').value;
  var planned=false;
  if(plannedDay){
    ...
    planned = targetType==='Doctor' ? plannedDocIds.includes(targetId) : plannedChemIds.includes(targetId);
...
function submitReport(){
  ...
  if (nonFieldWorkTypes.indexOf(repObj.workType) !== -1) {
    ...
    DB.products.forEach(function(p, i){
      var v=parseInt(document.getElementById('smp-'+i).value)||0;
      if(v>0){
        samples[p]=v;
        // Deduct inventory
    ...
    // Gifts deduction
    var gifts={};
    DB.gifts.forEach(function(g, i){
      var v=parseInt(document.getElementById('gft-'+i).value)||0;
      if(v>0){
        gifts[g]=v;
    ...
    // Inputs deduction
    var inputs={};
    DB.inputs.forEach(function(ip, i){
      var v=parseInt(document.getElementById('inp-'+i).value)||0;
      if(v>0){
        inputs[ip]=v;
  ...
  saveDB();
  showToast('Daily Call Report Submitted successfully!');
...
function renderMTPDaysUI(isReadOnly, existing){
  ...
  var teamIds = targetEmpId ? [String(targetEmpId).toUpperCase()] : null;

  if (targetEmpId) {
    var upperTargetId = String(targetEmpId).toUpperCase();
    
    // Populate directly from doctor assignments ONLY
    DB.doctors.forEach(function(d) {
      var assignedList = String(d.assignTo || '').toUpperCase().split(',').map(function(s){return s.trim();});
      var isAssignedToTeam = teamIds ? assignedList.some(function(a){ return teamIds.includes(a); }) : true;

      if (isAssignedToTeam && d.status === 'Active') {
        if (d.city) mrTowns.add(d.city.trim());
        if (d.area) mrAreas.add(d.area.trim());
      }
    });
  }
...
function sanitizeMTPDaysForAssignedMasters(){
  ...
  return currentMTPDays.filter(function(day){return !!day.areaTerritory;});
}
...
function saveOrSubmitTourPlan(status){
  ...
  if(!plannedDays.length){showToast('Please enter Area/Territory for at least one working day');return;}
...
function goTab(name){
  ...
  currentTab = name;
  ['home','report','tour','expense','leave','doctors','inventory'].forEach(function(t){
    var el=document.getElementById('tab-'+t);
    if (el) {
      if (t === 'report') {
        document.getElementById('scr-emp').insertBefore(el, document.querySelector('.nav'));
      }
...
function goAdminTab(name){
  ...
  ['dash','appr','master','inventory','reports','visit'].forEach(function(t){
...
function renderAdminTPApprovals(){
  ...
  if(!tps.length){el.innerHTML='<div class="empty">No tour plans pending your approval</div>';return;}
...
function renderAdminLeaveApprovals(){
  ...
  if(!lvs.length){el.innerHTML='<div class="empty">No leave requests pending your approval</div>';return;}
...
function viewTP(id){
  ...
  tp.days.forEach(function(d){
    var area=d.areaTerritory||d.toLocation||'';
    if(!area)return;
    var docIds=(d.plannedDocs&&d.plannedDocs.length) ? d.plannedDocs : DB.doctors.filter(function(doc){return String(doc.assignTo || '').toUpperCase() === String(tp.empId || '').toUpperCase() && doc.status==='Active' && doc.area.toLowerCase()===area.toLowerCase();}).map(function(doc){return doc.id;});
    var chemIds=(d.plannedChems&&d.plannedChems.length) ? d.plannedChems : DB.chemists.filter(function(chem){return String(chem.assignTo || '').toUpperCase() === String(tp.empId || '').toUpperCase() && chem.area.toLowerCase()===area.toLowerCase();}).map(function(chem){return chem.id;});
    html+='<tr><td class="tbl-name">'+d.date+'</td><td>'+area+'</td></tr>';
  });
...
function viewLeave(id){
  ...
  var html='<div style="margin-bottom:12px"><div style="font-size:15px;font-weight:700">'+lv.empName+' — '+lv.type+'</div>'+
           '<div style="margin-top:6px;font-size:13px"><strong>Start:</strong> '+lv.start+' &nbsp;|&nbsp; <strong>End:</strong> '+lv.end+' &nbsp;|&nbsp; <strong>Days:</strong> '+lv.days+'</div>'+
           '<div style="margin-top:8px;font-size:12px;background:#F1F5F9;padding:10px;border-radius:6px">Reason: '+lv.reason+'</div></div>';
           
  document.getElementById('leave-detail-content').innerHTML=html;
  
  var canApprove = isPendingForUser(lv.status, lv.managerId, u);
  document.getElementById('leave-approval-btns').style.display=canApprove?'flex':'none';
...
function renderSFCTable(){
  ...
  if(!filtered.length){
    tbody.innerHTML='<tr><td colspan="16" class="empty">'+(q ? 'No matching SFC records found' : 'No SFC records available')+'</td></tr>';
    return;
  }
  tbody.innerHTML=filtered.map(function(item){
    ...
    return '<tr><td><input type="checkbox" class="sfc-row-check admin-only" value="'+item.index+'" onchange="updateSelectedSFCCount()"></td>' +
           ...
           '<td>'+(s.doctors||'')+'</td>' +
           '<td>'+(s.business||'')+'</td>' +
           '<td><button class="btn sm danger admin-only" style="width:auto;padding:0 8px" onclick="removeSFC('+item.index+')">Delete</button></td></tr>';
  }).join('');
...
function uploadSFC(inp){
  ...
  reader.onload=function(e){
    ...
    dataLines.forEach(function(line){
      ...
      if(p.length<6||!p[4])return;
      var existing = DB.sfc.find(function(s){
        return s.from.toLowerCase() === p[4].toLowerCase() && s.to.toLowerCase() === p[5].toLowerCase();
      });
      var fare = parseFloat(p[9])||0;
      var da = parseFloat(p[10])||0;
      var total = parseFloat(p[11]) || (fare + da);

      if(existing){
        existing.workingDays = p[0]||'';
        existing.empName = p[1]||'';
        existing.hq = p[2]||'';
        existing.state = p[3]||'';
        existing.category = p[6]||'';
        existing.distance = parseInt(p[7])||0;
        existing.mode = p[8]||'';
        existing.fare = fare;
        existing.da = da;
        existing.total = total;
        existing.doctors = p[12]||'';
        existing.business = p[13]||'';
      } else {
        DB.sfc.push({
          workingDays: p[0]||'',
          empName: p[1]||'',
...
function downloadSFCTemplate(){
  var csv='WorkingDays,EmpName,HQ,State,From,To,Category,Distance,Mode,Fare,DA,Total,Doctors,Business\n';
  var checked = document.querySelectorAll('.sfc-row-check:checked');
  var list = [];
  if (checked.length > 0) {
...
function openEmployeeStatusModal(empId) {
  ...
  modalBody.innerHTML = [
    '<div class="info-box">',
      ...
      'Reporting Manager: ' + (mgr && mgr.name ? mgr.name : 'None'),
    '</div>',
    '<div class="btn-row" style="flex-wrap:wrap">',
      '<button class="btn warn sm" style="flex:1 1 calc(50% - 6px)" onclick="setEmployeeStatus(' + escapeHTML(JSON.stringify(emp.id) + ', ' + JSON.stringify('Hold')) + ')">Hold</button>',
      '<button class="btn danger sm" style="flex:1 1 calc(50% - 6px)" onclick="setEmployeeStatus(' + escapeHTML(JSON.stringify(emp.id) + ', ' + JSON.stringify('Resign')) + ')">Resign</button>',
      '<button class="btn gold sm" style="flex:1 1 calc(50% - 6px)" onclick="setEmployeeStatus(' + escapeHTML(JSON.stringify(emp.id) + ', ' + JSON.stringify('Replace')) + ')">Replace</button>',
      '<button class="btn success sm" style="flex:1 1 calc(50% - 6px)" onclick="setEmployeeStatus(' + escapeHTML(JSON.stringify(emp.id) + ', ' + JSON.stringify('Active')) + ')">Active</button>',
    '</div>'
  ].join('');
...
function bulkDeleteSelectedEmployees() {
  ...
  showToast(checked.length + ' employees deleted successfully!');
  var masterCheck = document.getElementById('check-all-emps');
  if (masterCheck) masterCheck.checked = false;
  updateSelectedEmpsCount();
}
...
function bulkDeleteAllEmployees() {
  ...
  showToast('All employee records (excluding ADMIN) deleted successfully!');
  var masterCheck = document.getElementById('check-all-emps');
  if (masterCheck) masterCheck.checked = false;
  updateSelectedEmpsCount();
}
...
function uploadDoctors(inp){
  ...
  reader.onload=function(e){
    ...
    dataLines.forEach(function(line){
      ...
      var assignToVal = (p[empIdCol]||'').toUpperCase();
...
function uploadChemists(inp){
  ...
  reader.onload=function(e){
    ...
    dataLines.forEach(function(line){
      ...
      var assignToVal = (p[assignCol]||'').toUpperCase();
...
function renderLeaveBalanceTable(){
  ...
  if(thead) {
    ...
    thead.innerHTML = '<tr><th colspan="8" style="border:none"></th><th colspan="12" style="text-align:center;border:none">'+monthYear+'</th><th style="border:none"></th></tr>' +
      '<tr><th rowspan="2"><input type="checkbox" id="check-all-leaves" class="admin-only" onclick="toggleSelectAllLeaves(this)"></th><th rowspan="2">S.No</th><th rowspan="2">Employee id</th><th rowspan="2">FieldForce Name</th><th rowspan="2">Designation</th><th rowspan="2">HQ</th><th rowspan="2">State</th><th rowspan="2">Joining Date</th><th colspan="4" style="text-align:center">Leave Eligibilty</th><th colspan="4" style="text-align:center">Leave Taken</th><th colspan="4" style="text-align:center">Leave Balance</th><th rowspan="2">Action</th></tr>' +
...
function downloadInventoryTemplate() {
  var samples = [
    ...
    'SERVIL-LS JUNIOR', 'SERVIL-LS SYRUP'
  ];
  var gifts = [
    ...
    emps.forEach(function(e) {
      ...
      samples.forEach(function(pName) {
        var stock = DB.samplesInventory.find(function(s) {
          return s.empId === e.id && s.prodName.toLowerCase().trim() === pName.toLowerCase().trim();
        });
      ...
      gifts.forEach(function(gName) {
        var stock = DB.giftsInventory.find(function(g) {
          return g.empId === e.id && g.giftName.toLowerCase().trim() === gName.toLowerCase().trim();
        });
        row.push(stock ? stock.opening : 0);
      });
      ...
      inputs.forEach(function(iName) {
        var stock = DB.inputsInventory.find(function(i) {
          return i.empId === e.id && i.inputName.toLowerCase().trim() === iName.toLowerCase().trim();
        });
        row.push(stock ? stock.opening : 0);
      });
...
function uploadInventoryCSV(inp) {
  ...
  reader.onload = function(e) {
    ...
    function classifyItem(itemName) {
      ...
      var matchedProd = DB.products.find(function(p) { return p.toLowerCase().trim() === nameLower; });
      if (matchedProd) return { type: 'sample', name: matchedProd };
      var matchedGift = DB.gifts.find(function(g) { return g.toLowerCase().trim() === nameLower; });
      if (matchedGift) return { type: 'gift', name: matchedGift };
      var matchedInput = DB.inputs.find(function(i) { return i.toLowerCase().trim() === nameLower; });
    ...
    if (hasCategoryRow && lines[1]) {
      isMultiColumn = true;
      dataStartIdx = 2;
      var row1 = parseCSVLine(lines[1]);
      var currentCategory = '';
      for (var i = 3; i < row1.length; i++) {
        if (row0[i] && row0[i].trim()) {
          var cat = row0[i].trim().toLowerCase();
          if (cat.indexOf('sample') !== -1 || cat.indexOf('product') !== -1) {
            currentCategory = 'sample';
          } else if (cat.indexOf('gift') !== -1) {
            currentCategory = 'gift';
          } else if (cat.indexOf('input') !== -1) {
    ...
    } else if (row0.length > 4 && (row0[0].toLowerCase().indexOf('emp') !== -1 || row0[0].toLowerCase().indexOf('employee') !== -1)) {
      isMultiColumn = true;
      dataStartIdx = 1;
    ...
    if (!isMultiColumn) {
      var hasHeader = lines[0].toLowerCase().indexOf('employeeid') !== -1;
      var dataLines = hasHeader ? lines.slice(1) : lines;
      ...
      dataLines.forEach(function(line) {
        ...
        var empId = (p[0] || '').trim();
        var itemType = (p[1] || '').trim().toLowerCase();
        var itemName = (p[2] || '').trim();
        var openingQty = parseInt(p[3]) || 0;
        ...
        if (itemType === 'sample' || itemType === 'product') {
          var matchedProd = DB.products.find(function(x) { return x.toLowerCase().trim() === itemName.toLowerCase().trim(); });
          if (matchedProd) {
          ...
          var matchedGift = DB.gifts.find(function(x) { return x.toLowerCase().trim() === itemName.toLowerCase().trim(); });
          if (matchedGift) {
          ...
          var matchedInput = DB.inputs.find(function(x) { return x.toLowerCase().trim() === itemName.toLowerCase().trim(); });
          if (matchedInput) {
      ...
      dataLines.forEach(function(line) {
        ...
        if (!matchedEmp) {
          console.warn("Bulk Upload: Employee ID " + empId + " not found. Skipping line.");
          return;
        }
        empId = matchedEmp.id;
...
function renderAdminInventoryTables(){
    ...
    document.getElementById('tbl-inv-samples').innerHTML=samplesList.length?samplesList.map(function(s){
      ...
      return '<tr><td><input type="checkbox" class="inv-row-check" data-type="sample" value="' + idVal + '"></td><td>'+(emp?emp.name:'')+' ('+s.empId+')</td><td>'+s.prodName+'</td><td>'+s.opening+'</td><td>'+s.received+'</td><td>'+s.distributed+'</td><td style="font-weight:700">'+s.balance+'</td><td><button onclick="deleteInventoryItem(\'sample\', \'' + idVal + '\')" class="btn sm danger" style="padding:4px 8px">Delete</button></td></tr>';
    }).join(''):'<tr><td colspan="8" class="empty">No entries found</td></tr>';
    ...
    document.getElementById('tbl-inv-gifts').innerHTML=giftsList.length?giftsList.map(function(g){
      ...
      return '<tr><td><input type="checkbox" class="inv-row-check" data-type="gift" value="' + idVal + '"></td><td>'+(emp?emp.name:'')+' ('+g.empId+')</td><td>'+g.giftName+'</td><td>'+g.opening+'</td><td>'+g.received+'</td><td>'+g.distributed+'</td><td style="font-weight:700">'+g.balance+'</td><td><button onclick="deleteInventoryItem(\'gift\', \'' + idVal + '\')" class="btn sm danger" style="padding:4px 8px">Delete</button></td></tr>';
    }).join(''):'<tr><td colspan="8" class="empty">No entries found</td></tr>';

    // Render Inputs
    var inputsList = DB.inputsInventory.filter(function(i){return !mrFilter||i.empId===mrFilter;});
    document.getElementById('tbl-inv-inputs').innerHTML=inputsList.length?inputsList.map(function(i){
      ...
      return '<tr><td><input type="checkbox" class="inv-row-check" data-type="input" value="' + idVal + '"></td><td>'+(emp?emp.name:'')+' ('+i.empId+')</td><td>'+i.inputName+'</td><td>'+i.opening+'</td><td>'+i.received+'</td><td>'+i.distributed+'</td><td style="font-weight:700">'+i.balance+'</td><td><button onclick="deleteInventoryItem(\'input\', \'' + idVal + '\')" class="btn sm danger" style="padding:4px 8px">Delete</button></td></tr>';
    }).join(''):'<tr><td colspan="8" class="empty">No entries found</td></tr>';
