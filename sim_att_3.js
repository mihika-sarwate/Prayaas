...
const dom = new JSDOM(`<body>
  <input id="attendance-employee-filter" value="TEST03" />
...
async function run() {
  await c.connect();
  const empRes = await c.query("SELECT * FROM employees WHERE id = 'TEST03'");
  const repRes = await c.query("SELECT * FROM reports WHERE emp_id = 'TEST03'");
