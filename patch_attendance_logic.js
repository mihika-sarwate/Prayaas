...
files.forEach(f => {
  ...
  // 1. Patch finalSubmitToday
  const submitRegex = /saveDB\(\);\s*var pushPromises = myTodayReports\.map\(function\(r\) \{\s*return pushReportToSupabase\(r\)\.catch\(function\(e\) \{ console\.error\(e\); \}\);\s*\}\);/;
  if (c.match(submitRegex)) {
    const newSubmitLogic = `
  saveDB();
  
  // NEW LOGIC: Immediate Attendance Update
  var attendanceId = 'ATT-' + u.id + '-' + today;
  if (!DB.attendance) DB.attendance = [];
  var existingAtt = DB.attendance.find(function(a){return a.id === attendanceId;});
  ...
  if (!existingAtt) {
    ...
    DB.attendance.push(existingAtt);
  } else {
    existingAtt.attendanceStatus = 'P';
    existingAtt.remarks = 'Present via Final DCR Submission';
  }
...
`;
    c = c.replace(submitRegex, newSubmitLogic.trim());
    updated = true;
  }

  // 2. Patch getAttendanceDashboardRows
