...
const files = ['live_index.html', 'script_0.js', 'temp_script_0.js', 'temp_script_1.js'];
...
files.forEach(f => {
  ...
  const startIdx = c.indexOf('function getAttendanceDashboardRows() {');
  if (startIdx === -1) return;
  
  // Find the end pattern (could be \n or \r\n)
  const endPatternLF = '  });\n}';
  const endPatternCRLF = '  });\r\n}';
  let endIdx = c.indexOf(endPatternLF, startIdx);
  let endPattern = endPatternLF;
  
  if (endIdx === -1) {
    endIdx = c.indexOf(endPatternCRLF, startIdx);
    endPattern = endPatternCRLF;
  }
  
  if (endIdx === -1) return;
  
  const oldFunc = c.substring(startIdx, endIdx + endPattern.length);
  
  if (oldFunc.indexOf('finalDcrExists') !== -1) {
  ...
  c = c.replace(oldFunc, newDashLogic);
  fs.writeFileSync(f, c);
  console.log(f + ' successfully FORCE updated.');
});
