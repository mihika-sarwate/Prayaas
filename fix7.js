...
// Insert the functions after saveDB
c = c.replace(/function saveDB\([^)]*\)\s*\{[\s\S]*?return;\s*\}/, match => match + '\n\n' + persistAssignCode);
...
c = c.replace(/assignTo:\s*document\.getElementById\('md-assign'\)\.value,/, `assignTo: document.getElementById('md-assign').value,`);
// Also need to persist for NEW doctors
c = c.replace(/DB\.doctors\.push\(\{([\s\S]*?)id:\s*docId,([\s\S]*?)assignTo:\s*document\.getElementById\('md-assign'\)\.value,([\s\S]*?)\}\);/, 
...
c = c.replace(/DB\.chemists\.push\(\{([\s\S]*?)id:\s*chemId,([\s\S]*?)assignTo:\s*document\.getElementById\('mc-assign'\)\.value,([\s\S]*?)\}\);/,
  (match) => match + `\n    persistLocalAssignTo('chemists', chemId, document.getElementById('mc-assign').value);`
);
...
c = c.replace(/DB\.stockists\.push\(\{([\s\S]*?)id:\s*stkId,([\s\S]*?)assignTo:\s*document\.getElementById\('ms-assign'\)\.value,([\s\S]*?)\}\);/,
  (match) => match + `\n    persistLocalAssignTo('stockists', stkId, document.getElementById('ms-assign').value);`
);
...
console.log('Persist logic injected');
